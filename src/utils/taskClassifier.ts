...
export interface ContextNeeds {
  /** Nexus SOP + 规则引擎 + 性能洞察 + nexusCommunicationHint + sopEvolution */
  nexusSOP: boolean
  /** L0 核心记忆 + Nexus 最近记忆 */
  memory: boolean
  /** 历史成功案例 + exec_trace */
 ...
 *
 * @param query - 用户输入
 * @param matchedNexus - nexusManager.matchForTask() 自动匹配的 Nexus（可能为 null）
 * @param activeNexusId - 用户当前所在的 Nexus 会话 ID（通过 nexusId 参数或 getActiveNexusId）
...
export function classifyTaskComplexity(
  ...
  const hasNexus = !!matchedNexus || !!activeNexusId

  if (hasNexus) {
    signals.nexusScore += 1
    if (matchedNexus?.sopContent) {
      signals.nexusScore += 1
    }
  }
