...
├── scripts/             # 运行脚本（按用途分类）
│   ├── analysis/        # 数据分析脚本
│   ├── generate/        # 文档/PPT 生成脚本
│   ├── debug/           # 调试和测试脚本
│   ├── fix/             # 修复和工具脚本
│   └── setup/           # 环境配置和构建脚本
├── skills/              # 内置技能定义 (SKILL.md 文件)
├── nexuses/             # 内置 Nexus 定义（开发用）
├── registry/            # 技能和 MCP 注册表
├── public/              # 静态资源
├── duncrew-server.py    # Python 本地服务器 (工具执行后端)
└── 配置文件             # package.json, vite.config.ts, tsconfig.json 等

数据目录 (DunCrew-Data/, 由 clawd_path 配置)
├── nexuses/{nexus-id}/  # Nexus 定义和数据
│   ├── NEXUS.md         # Nexus 定义文件
│   ├── experience/      # 执行经验记录 (successes.md, failures.md)
│   └── output/          # Nexus 执行任务的产出文件
├── documents/           # 非 Nexus 产生的文档和报告
│   ├── reports/         # 分析报告
│   ├── data/            # 数据文件
│   └── 产品素材/        # 产品宣传素材
├── skills/              # 用户技能文件 (优先级高于内置)
├── memory/              # 记忆系统
│   └── exec_traces/     # 执行追踪日志
├── logs/                # 运行日志
└── duncrew.db           # SQLite 持久化存储
```
